...
```{r graph_functions, message=FALSE}
...
```{r community_detection_functions, message=FALSE}
...
```{r node_based_analysis, warning=FALSE}
#node-measures
...
graph_discp_data <- function(nd) {
  ...
  split %>%
    gather(key, value, dg:eigen, discp_hmph:cntry_hmph) %>%
...
#missing the first 4 data sets currently due to lack of discipline information in current dataset
for(i in 5:length(node.data)) {
