...
graph_network(generate_csss_network_df(df = clean_all, iter = "2011.Santa Fe", colorByDisc = TRUE, colorEdges = TRUE, hideNames = TRUE))
...
filename.list <- paste0("csss_", str_replace(str_replace(iter.list, pattern = " ", ""), pattern = "\\.", replacement = "_"), "_nw.png")
filebasepath <- "./img/"
...
nd <- nodes %>%
  filter(discp %in% c("Computing", "Mathematics and statistics", "Physical sciences", "Life sciences", "Social and behavioural science")) %>%
  group_by(discp, year) %>%
